...
| 指标 | v4 冻结候选 | 零转向直行基线 |
|---|---:|---:|
| 平均终点纵坐标 | 41.55 m | 49.15 m |
| 平均通过障碍数 | 2.50 / 9 | 3.19 / 9 |
| 首障碍通过率 | 81.25% | 81.25% |
| 通关率 | 0% | 0% |

候选的首障碍通过前碰撞率为 18.75%，驶出道路率为 0%。原始转向镜像误差、
执行转向镜像误差、配对里程差和终止步数差均为 0；左、右首障碍组表现完全一致。
固定侧偏得到消除，但这不等于获得正确的避障响应。平均仅通过 2.5 个障碍，
并且没有超过简单直行基线，因此**可用性门槛未通过，不发布默认策略**。

对称性由工程约束实现：原始图像和镜像图像各驱动一份持续的全连接组状态，
共享结构增益，转向读取奇分量，油门与 MDN 后退读取偶分量。不是把原始生物
连接组视为严格对称，也不是隐藏规则避障器；每个微步需要两次全图传播。
道路安全层仍不读取障碍，只使用位置、速度、航向与预测横向位置。

同一候选的安全层消融已完成：关闭→开启后，平均终点纵坐标 `39.79→41.55m`、
通过障碍数 `2.375→2.5`、驶出道路率 `12.5%→0%`；首障碍前碰撞率两组均为
`18.75%`，通关率两组均为 `0%`。因此零出界并非全部由新增的首障碍碰撞造成，
但仍不能证明可用避障。距离差 `+1.76m` 的道路对 bootstrap 区间为
`[0.0029, 5.0975]`，下界接近零，应谨慎解读；整体仍不如直行基线。
...
- `artifacts/stable-policy-calibration.json`：完整校准、镜像分组、逐步轨迹、直行基线与门槛。
- `artifacts/mirror-constraint-ablation.json`：同一候选的安全层开关消融。
- `artifacts/checkpoints/driving-policy.calibrated-candidate.npz`：保留的 v4 失败候选，不自动加载。
- 候选 SHA-256：`452271404ea9e064e2a697ae83b04c8434b8fb07fd20256009c2c12f226cdd60`。

旧 v2 默认检查点保留为历史文件，但当前加载器明确停用它，界面显示“旧检查点
已停用，当前策略未校准”。v4 不会默默迁移旧归一化状态，也不会把失败候选
包装成可用策略。完整协议见 [驾驶实验](docs/driving-experiment.md)。
...
仓库保留旧 v2 检查点，但 v4 不加载它。页面会明确显示未校准状态。点击
“恢复发布策略”会撤销进程内实验更新；在没有兼容发布策略时，恢复为未校准策略。
