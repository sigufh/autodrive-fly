...
> 均等待视觉门槛通过。来源审计发现方向相关的解剖结构，但不是功能学习证据。
> 本轮确认 R1–R6 左右覆盖为 1,107/2,237；保持全部受体、仅归一化眼侧总输入，
> 修订后的镜像误差从 0.435 降至 0.286，仍未过 0.20。957/眼共同视柱子集虽通过
> 该镜像单项，却使 T5 极性对比全部为负；T4 方向仍失败，不修改默认视网膜。
> 镜像评分是在发现静默响应放大后修订的，不能替代独立验证；拟合参数也未进入
> 该实验的 test 刺激评估。详见
