...
> 均等待视觉门槛通过。独立 optic-hex 轴交叉验证发现 T5 零样本方向信号
> （83.5%）。进一步的 T4 来源嵌套验证定位了 Mi4/C3 与 Mi9 聚合错误，解剖方向
> 可达 96.38%，但冻结三支路模型在真实视觉电池中 T4 方向对比仍仅 0.0024，
> 镜像误差 0.5053；公开参数的单室电导模型也未能迁移到当前无量纲动力学，
> 严格拆分的表型约束拟合在未见细胞/刺激上也只有 51.6% 方向准确率，test 保持
> 密封，因此继续阻断、没有进入中央复合体。详见
> [v7 路线与阶段报告](docs/driving-v7.zh-CN.md)。
