...
> v7 已开始构建，但当前阻断在受控视觉阶段：线性亮度/逐帧差分结合类型化动力学
> 已恢复 ON/OFF 分化和部分 LPLC/LC looming 时序响应，但仍未恢复 T4/T5 方向
> 选择与左右镜像一致性，也未证明真实拓扑优于
> 三类初筛打乱控制。v7 未部署，中央复合体、DNa 读出、蘑菇体学习和城市扩张
> 均等待视觉门槛通过。详见 [v7 路线与阶段报告](docs/driving-v7.zh-CN.md)。
...
身体感觉也已从同侧全组广播拆成 10 个解剖子组，共覆盖 474 个互不重叠节点，7 个
注释不足节点不刺激。低增益组合保持任务能力并降低转向，但相对新全景底座略增每米
漂移，因此默认身体增益仍为 0，等待与 DNp20 慢适应的交互消融。证据：
`artifacts/neural-v6-body-sensory-ablation.json`。

2×2 交互消融进一步确认：身体反馈不能替代 DNp20 8% 慢适应；两者同时开启虽继续
100% 通关并降低转向，却把每米漂移 0.0638 提高到 0.0673。因此保留 8% 适应器、
默认身体增益为 0。证据：`artifacts/neural-v6-body-motor-interaction.json`。

感觉通路可塑性现限制在 11 个镜像绑定增益，覆盖 7,226 个感觉源神经元的
540,845 条既有出边，运动端权重和动作探索冻结。最小逐级课程保持 100% 通关，
但转向和每米漂移均变差，因此候选被拒绝、不进入扩大评估。证据：
`artifacts/neural-v6-sensory-pathway-curriculum.json`。

最终在全新 `1000–1015`（8 对镜像道路）上扩大验证：坐标保持全景把完整窗口
转向 0.2194 降到 0.2156、每米漂移 0.1133 降到 0.1009，但通关率 50%→37.5%，
出界率 0%→12.5%。因此未通过发布门槛，线上继续使用 `front + v6 + 8% DNp20
适应`。证据：`artifacts/neural-v6-panorama-release.json`。
