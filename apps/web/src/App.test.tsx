...
test('renders hash-verified offline v7 gates without presenting v7 as runtime', async () => {
  vi.mocked(fetchV7Status).mockResolvedValueOnce({
    ...
    evidence_boundaries: { nine_source_contract_complete: false, Mi4_C3_direct_numeric_voltage_candidates: ['Groschner_2022'], Mi4_C3_independent_numeric_voltage_candidate_count: 0, T5_voltage_field_counts: { aggregated_full_field_OFF_flash: 7, raw_white_noise: 8, raw_drifting_grating: 9 }, T5_record_specific_stimulus_logs_available: false, Motyxia2_public_history_branch_count: 22, Motyxia2_public_history_commit_count: 447, T5_record_log_found_in_Motyxia2_public_history: false, T5_external_successful_indexes_linked_log_found: false, T5_PMC_supplement_content_inspected: false, T5_Figshare_search_accessible: false, T5_stimulus_log_global_absence_claimed: false, T5_generator_defaults_used_as_record_fields: false, T5_stimulus_provenance_complete: false },
  ...
  expect(screen.getByText('外部索引：linked log 未命中；PMC supplement 未检查；Figshare 不可访问；全局不存在未声明')).toBeInTheDocument()
