...
export function DrivingPanel({ state, running, learning, explore, safetyConstraints, controlMode, onControlMode, onLearning, onExplore, onSafetyConstraints, onRun, onStep, onReset }: {
  ...
  return <section className="driving-panel" aria-label="果蝇视觉驾驶">
    ...
    <p className="scientific-note">{controlMode === 'neural' ? state?.policy_checkpoint.loaded ? '已加载 MaleCNS v6：当前保留 143° 前视与 DNp20 8% 慢基线适应。v7 已独立启动，但受控视觉基准尚未恢复 T4/T5 方向选择和 LPLC/LC looming 响应，真实拓扑也未胜过初筛打乱控制，因此阻断在视觉动力学阶段、未进入导航或部署。' : '纯神经决策：DNp20/DNpe017/MDN 输出直接映射为车辆动作；障碍、道路和规则仅作为视觉刺激、奖励与结果反馈。' : state?.policy_checkpoint.loaded ? `工程避障基线：已加载 ${state.policy_checkpoint.kind === 'frozen_calibrated' ? '冻结校准' : '实验学习'}策略。` : state?.policy_checkpoint.rejection ? '旧检查点已停用，当前策略未校准。' : '当前为未训练策略。'}</p>
  </section>
}
