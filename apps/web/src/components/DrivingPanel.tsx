...
export function DrivingPanel({ state, running, learning, explore, safetyConstraints, controlMode, onControlMode, onLearning, onExplore, onSafetyConstraints, onRun, onStep, onReset }: {
  ...
  return <section className="driving-panel" aria-label="果蝇视觉驾驶">
    ...
    <p className="scientific-note">{controlMode === 'neural' ? state?.policy_checkpoint.loaded ? '已加载 MaleCNS v6：当前默认服务仍为 v5/v6 发布基线。独立 v7 闭环候选只经 R1–R6、T4/T5 空间分桶与固定事件动作映射，在单障碍 tuning 中 6/6、一次 calibration 中 2/2 成功；零动作和错误符号均为 0/6，点采样仅 2/6。冻结候选在九障碍诊断中为 0/6 完成、每局仅通过 0–3 个障碍，因此尚未进入 OOD、外部 final 或导航发布。' : '纯神经决策：DNp20/DNpe017/MDN 输出直接映射为车辆动作；障碍、道路和规则仅作为视觉刺激、奖励与结果反馈。' : state?.policy_checkpoint.loaded ? `工程避障基线：已加载 ${state.policy_checkpoint.kind === 'frozen_calibrated' ? '冻结校准' : '实验学习'}策略。` : state?.policy_checkpoint.rejection ? '旧检查点已停用，当前策略未校准。' : '当前为未训练策略。'}</p>
  </section>
}
