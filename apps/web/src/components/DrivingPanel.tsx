...
export function DrivingPanel({ state, running, learning, explore, safetyConstraints, controlMode, onControlMode, onLearning, onExplore, onSafetyConstraints, onRun, onStep, onReset }: {
  ...
  return <section className="driving-panel" aria-label="果蝇视觉驾驶">
    ...
    <p className="scientific-note">{controlMode === 'neural' ? state?.policy_checkpoint.loaded ? '已加载 MaleCNS v6：默认服务仍为 v5/v6 发布基线。独立 v7 结构化神经 readout 仅经 R1–R6、T4/T5 空间区及 LPLC/LC 时空统计，在九障碍 tuning 6/6、一次 calibration 2/2（18/18 障碍）。消融：去 T4/T5 为 0/6，去 LPLC2 为 4/6，去全部 LPLC/LC 为 2/6，去 heading 反馈为 0/6。它仍是教师监督工程解码器，未通过细胞生理、拓扑优势、OOD 或外部 final，不替换发布服务。' : '纯神经决策：DNp20/DNpe017/MDN 输出直接映射为车辆动作；障碍、道路和规则仅作为视觉刺激、奖励与结果反馈。' : state?.policy_checkpoint.loaded ? `工程避障基线：已加载 ${state.policy_checkpoint.kind === 'frozen_calibrated' ? '冻结校准' : '实验学习'}策略。` : state?.policy_checkpoint.rejection ? '旧检查点已停用，当前策略未校准。' : '当前为未训练策略。'}</p>
  </section>
}
