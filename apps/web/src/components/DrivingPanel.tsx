...
export function DrivingPanel({ state, running, learning, explore, safetyConstraints, controlMode, onControlMode, onLearning, onExplore, onSafetyConstraints, onRun, onStep, onReset }: {
  ...
  return <section className="driving-panel" aria-label="果蝇视觉驾驶">
    ...
    <p className="scientific-note">{controlMode === 'neural' ? state?.policy_checkpoint.loaded ? '已加载 MaleCNS v6：默认服务仍为 v5/v6 发布基线。v7 单障碍 T4/T5 空间读出 tuning 6/6、calibration 2/2，但冻结候选在九障碍中 0/6。质量守恒 balanced R1–R6 的局部危险上界可精确重建视觉输入，并在九障碍 tuning 6/6、一次 calibration 2/2（18/18 障碍）；这只证明输入与透明动作反馈足够，尚未证明 T4/T5、LPLC/LC 神经核、OOD 或外部 final。' : '纯神经决策：DNp20/DNpe017/MDN 输出直接映射为车辆动作；障碍、道路和规则仅作为视觉刺激、奖励与结果反馈。' : state?.policy_checkpoint.loaded ? `工程避障基线：已加载 ${state.policy_checkpoint.kind === 'frozen_calibrated' ? '冻结校准' : '实验学习'}策略。` : state?.policy_checkpoint.rejection ? '旧检查点已停用，当前策略未校准。' : '当前为未训练策略。'}</p>
  </section>
}
