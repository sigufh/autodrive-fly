...
export function DrivingPanel({ state, running, learning, explore, safetyConstraints, controlMode, onControlMode, onLearning, onExplore, onSafetyConstraints, onRun, onStep, onReset }: {
  ...
  return <section className="driving-panel" aria-label="果蝇视觉驾驶">
    ...
    <p className="scientific-note">{controlMode === 'neural' ? state?.policy_checkpoint.loaded ? '已加载 MaleCNS v6：当前保留已验证的 143° 前视与 DNp20 慢基线适应。近全景、T4/T5 光流和平衡器/本体感觉已实现并完成消融，但尚未同时保持任务和稳定性，因此未部署。' : '纯神经决策：DNp20/DNpe017/MDN 输出直接映射为车辆动作；障碍、道路和规则仅作为视觉刺激、奖励与结果反馈。' : state?.policy_checkpoint.loaded ? `工程避障基线：已加载 ${state.policy_checkpoint.kind === 'frozen_calibrated' ? '冻结校准' : '实验学习'}策略。` : state?.policy_checkpoint.rejection ? '旧检查点已停用，当前策略未校准。' : '当前为未训练策略。'}</p>
  </section>
}
