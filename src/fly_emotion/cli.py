...
from .driving.v7_afterimages_mi4_candidate_audit import (
    evaluate_v7_afterimages_mi4_candidate_audit,
)
...
from .driving.v7_t4_source_pool_camera_frame_replication import (
    evaluate_v7_t4_source_pool_camera_frame_replication,
)
...
from .driving.v7_t5_increment_order_control_replication import (
    evaluate_v7_t5_increment_order_control_replication,
)
...
def _parser() -> argparse.ArgumentParser:
    ...
    subparsers.add_parser("v7-audit-afterimages-mi4-candidate")
    ...
    subparsers.add_parser("v7-evaluate-t4-source-pool-camera-frame-replication")
    ...
    subparsers.add_parser("v7-evaluate-t5-increment-order-control-replication")
...
def main() -> None:
    ...
    if args.command == "v7-audit-tanaka-mi4-calcium":
        ...
        print(target)
        return
    if args.command == "v7-audit-afterimages-mi4-candidate":
        report = evaluate_v7_afterimages_mi4_candidate_audit(root)
        target = root / "artifacts/v7-afterimages-mi4-candidate-audit.json"
        target.write_text(
            json.dumps(report, indent=2, allow_nan=False) + "\n", encoding="utf-8"
        )
        print(target)
    ...
    if args.command == "v7-freeze-t4-source-pool-camera-frame-replication":
        ...
        print(target)
        return
    if args.command == "v7-evaluate-t4-source-pool-camera-frame-replication":
        report = evaluate_v7_t4_source_pool_camera_frame_replication(root)
        target = root / "artifacts/v7-t4-source-pool-camera-frame-replication.json"
        target.write_text(
            json.dumps(report, indent=2, allow_nan=False) + "\n", encoding="utf-8"
        )
        print(target)
    ...
    if args.command == "v7-freeze-t5-increment-order-control-replication":
        ...
        print(target)
        return
    if args.command == "v7-evaluate-t5-increment-order-control-replication":
        report = evaluate_v7_t5_increment_order_control_replication(root)
        target = root / "artifacts/v7-t5-increment-order-control-replication.json"
        target.write_text(
            json.dumps(report, indent=2, allow_nan=False) + "\n", encoding="utf-8"
        )
        print(target)
