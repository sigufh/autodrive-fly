...
def test_v7_status_is_hash_verified_and_explicitly_not_deployed() -> None:
    ...
    assert boundaries["Gonzalez_Suarez_2022_bioRxiv_supplement_retrieved"] is True
    assert boundaries["Gonzalez_Suarez_2022_individual_flies_are_statistical_units"] is True
    assert boundaries["Gonzalez_Suarez_2022_public_individual_numeric_payload"] is False
    ...
    assert boundaries["Strother_2018_author_public_repository_count"] == 2
    assert boundaries["Strother_2018_author_repository_numeric_payload_verified"] is False
    assert boundaries["Strother_2018_author_repository_search_is_bounded"] is True
    ...
    assert boundaries["Hao_2026_Wayback_capture_view"] == "abstract_only"
    assert boundaries["Hao_2026_Wayback_linked_route_contents_retrieved"] is False
    assert boundaries["Hao_2026_Wayback_resolves_complete_cell_types"] is False
    assert boundaries["Hao_2026_ClandininLab_public_repository_count"] == 31
    assert boundaries["Hao_2026_ClandininLab_paper_repository_hits"] == []
    assert boundaries["Hao_2026_bioRxiv_TDM_requester_pays_required"] is True
    assert boundaries["Hao_2026_bioRxiv_TDM_payload_inventory_readable"] is False
    ...
    assert boundaries["fig3_source_kernel_baseline_or_gain_mismatch_explains_failure"] is False
    assert boundaries["fig3_source_kernel_oracle_lag_milliseconds"] == 250
    ...
    assert boundaries["MaleCNS_Tm4_synapse_count_right_hex_distance_counts"] == {
        "0": 28,
        "1": 5,
        "2": 14,
        "3": 1,
    }
    assert boundaries["MaleCNS_Tm4_synapse_count_right_within_one_hex_fraction"] == (
        33 / 48
    )
