...
def test_saved_goal_audit_is_hash_bound_and_matches_recalculation() -> None:
    ...
    assert np.isclose(
        visual["observations"]["T5_Tm2_oracle_shift_LOO_median_correlation"],
        0.806123379293452,
    )
    assert np.isclose(
        visual["observations"][
    ...
    assert visual["observations"]["T5_Tm2_oracle_maximum_lag_milliseconds"] == 250
    assert (
        visual["observations"]["T5_Tm2_oracle_alignment_all_shape_gates_passed"]
        is False
    )
    assert (
        visual["observations"][
    ...
    assert visual["observations"]["Hao_2026_bioRxiv_TDM_requester_pays_required"] is True
    assert visual["observations"]["Hao_2026_bioRxiv_TDM_payload_inventory_readable"] is False
    ...
    assert visual["observations"]["MaleCNS_Tm4_post_hoc_hex_tolerance_authorized"] is False
