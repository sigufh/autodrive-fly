...
REPORT = ROOT / "artifacts/v7-t4-synapse-centered-precheck.json"


def test_T4_synapse_centered_candidate_is_frozen_and_does_not_modify_conductance() -> None:
    ...
    assert report["protocol"]["conductance_base_modified"] is False
    assert report["protocol"]["runtime_modified"] is False


def test_T4_synapse_centering_does_not_restore_bilateral_direction() -> None:
    report = json.loads(REPORT.read_text())
...
def test_T4_synapse_centered_stop_rule_closes_single_step_family() -> None:
    report = json.loads(REPORT.read_text())
    ...
    assert report["stop_reason"] == (
        "no_bilateral_T4_direction_subtype_passed_centered_main_gate"
    )
    assert report["boundary"]["source_centering_does_not_modify_brain_state"] is True
